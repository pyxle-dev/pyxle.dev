# Pyxle — The backend and the page. One file.

Pyxle is a Python web framework where the data loader and the React page are the same file — so the API layer between them simply doesn't exist. You write `@server` loaders and `@action` mutations in Python right beside real React components (server-rendered and hydrated) in a single `.pyxl` file — file-based routing, validated actions, streaming SSR.

If it runs Python and Node, it runs Pyxle.

## Start here

- [Introduction](https://pyxle.dev/docs/getting-started/introduction.md) — what Pyxle is and why
- [Installation](https://pyxle.dev/docs/getting-started/installation.md)
- [Quick start](https://pyxle.dev/docs/getting-started/quick-start.md) — build your first app
- [.pyxl files](https://pyxle.dev/docs/core-concepts/pyxl-files.md) — the core format
- [Pyxle Studio](https://pyxle.dev/docs/guides/studio.md) — the dashboard built into `pyxle dev`
- [Debugging](https://pyxle.dev/docs/guides/debugging.md) — breakpoints in the loader and the component, in one file
- [Editor setup](https://pyxle.dev/docs/guides/editor-setup.md) — the VS Code extension: `code --install-extension pyxle.pyxle-language-tools`
- [Pyxle for AI coding agents](https://pyxle.dev/docs/guides/for-ai-agents.md)

## Explore

- Full docs index: https://pyxle.dev/llms.txt
- Entire docs as one file: https://pyxle.dev/llms-full.txt
- Benchmarks and methodology: https://pyxle.dev/benchmarks.md
- Interactive playground: https://pyxle.dev/playground.md
- Roadmap: https://pyxle.dev/roadmap.md
